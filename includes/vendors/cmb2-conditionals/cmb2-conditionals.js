jQuery( document ).ready( function( $ ) {
	...
	function CMB2ConditionalsInit( changeContext, conditionContext ) {
		...
		changeContext.on( 'change', 'input, textarea, select', function( evt ) {
           ...
           var target_group_par = $( this ).parents( '.cmb-repeat-group-wrap:first' );
           if (typeof target_group_par[0] == "undefined"){
			...
			if ( dependants.length > 0 ) {
				...
				dependants.each( function( i, e ) {
					...
					if ( 'undefined' !== typeof currentFieldName && '' !== currentFieldName && $.inArray( currentFieldName, dependantsSeen ) < 0 ) {
                        ...
                        current.parents( '.cmb-repeat-group-wrap:first' ).toggle( shouldShow );
        ...
        $( '[data-conditional-id]', conditionContext ).parents( '.cmb-repeat-group-wrap:first' ).hide();
		...
		$( '.cmb2-wrap > .cmb2-metabox' ).on( 'cmb2_add_row', function( evt, row ) {
           ...
           // $( '[data-conditional-id]', row ).parents( '.cmb-repeat-group-wrap:first' ).hide();
	...
	function CMB2ConditionalsRecursivelyHideDependants( fieldName, elm, context ) {
		...
		if ( dependants.length > 0 ) {
			dependants.each( function( i, e ) {
                ...
                dependant.parents( '.cmb-repeat-group-wrap:first' ).hide();
	...
	CMB2ConditionalsInit( '.cmb2-wrap', '.cmb2-wrap .cmb2-metabox' );
});
